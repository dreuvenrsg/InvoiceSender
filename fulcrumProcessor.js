...
async function clickNeedsAction(page) {
  ...
  await page.waitForTimeout(config.timeouts.pageStabilization);
...
async function processCreate(page, row, rowData, errors) {
  try {
    ...
    if (!issuedClicked) throw new Error('Issued button not found');
    await page.waitForTimeout(1000);
    
    // Confirm modal (click OK)
    console.log('[Row] Confirming...');
    await page.waitForSelector('.fulcrum-modal', { visible: true, timeout: config.timeouts.elementWait });
    
    const okClicked = await page.evaluate(() => {
      const modal = document.querySelector('.fulcrum-modal');
      if (modal) {
        const buttons = Array.from(modal.querySelectorAll('button'));
        const okBtn = buttons.find(btn => btn.textContent.trim() === 'Ok' && btn.classList.contains('btn-primary'));
        if (okBtn) {
          okBtn.click();
          return true;
        }
    ...
    await page.waitForTimeout(config.timeouts.modalWait);
    console.log(`[Row] ✓ ${rowData.soNumber} created and issued`);
    
    // Go back to invoicing page and re-click NEEDS ACTION
...
// Process a row with "Issue" button (Issue → Cancel workflow)
...
async function processPage(page, processedInvoices, errors) {
  ...
  try {
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      await page.waitForTimeout(1000); // Small delay between rows
    }
